...
const createCheckableData = (data: {
  [key: string]: string
}[] = []) => {
  return data.map(v => {
...
const BaseTable: React.FC<IBaseTableProperties> = (props) => {
  ...
  const handleSelectedRow = (_event: any, row: KeyTypeString, checked: boolean) => {
    const newData = tempData.map((v: KeyTypeString) => {
      ...
      const rowSelectionKey2 = rowSelection && rowSelection?.selector ? rowSelection.selector as keyof typeof row : '';
        ...
        <TableRow>
          {isRowSelection && (
            <TableCell align="left">
              <Checkbox checked={isCheckedAll} indeterminate={!isCheckedAll && hasChecked} onChange={handleCheckedAll} />
            </TableCell>
          )}
          ...
          <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
            {isRowSelection && (
              <TableCell component="th" scope="row" width={20} align="left">
                <Checkbox checked={Boolean(row && row?.checked)} onChange={(e, checked) => handleSelectedRow(e, row, checked)} />
              </TableCell>
            )}
