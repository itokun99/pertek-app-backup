...
import { getTenant, createTenant, updateTenant, deleteTenant, getTenantById } from '@backend/repos/tenant';
...
async function handlerGet(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [response, payload] = req.query?.id ? await getTenantById(req) : await getTenant(req);
...
async function handlerPost(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [response, payload] = await createTenant(req);
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [response, payload] = await updateTenant(req);
...
async function handlerDelete(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [response, payload] = await deleteTenant(req);
