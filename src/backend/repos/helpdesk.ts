...
import { ApiResponseType, IHelpdesk } from "@general-types";

// create function to get list of helpdesk
export async function getHelpdesk(
  req: NextApiRequest
): Promise<[Response, ApiResponseType<IHelpdesk>]> {
...
export async function createHelpdesk(
  req: NextApiRequest
): Promise<[Response, ApiResponseType<IHelpdesk>]> {
  const apiResponse = await apiRequest({
    req,
    url: Endpoint.Helpdesk,
    method: "POST",
    body: req.body,
  });
...
export async function updateHelpdesk(
  req: NextApiRequest
): Promise<[Response, ApiResponseType<IHelpdesk>]> {
  const { id } = req.query;
  const apiResponse = await apiRequest({
    req,
    url: `${Endpoint.Helpdesk}/${id}`,
    method: "PUT",
    body: req.body,
  });
  const responseBody = await apiResponse.json();
...
export async function deleteHelpdesk(
  req: NextApiRequest
): Promise<[Response, ApiResponseType<IHelpdesk>]> {
  const { id } = req.query;
  const apiResponse = await apiRequest({
