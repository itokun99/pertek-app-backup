...
import { ApiResponseType, IAnnouncement } from "@general-types";

export async function getAnnouncement(
  req: NextApiRequest
): Promise<[Response, ApiResponseType<IAnnouncement>]> {
  const params = createRequestParams(req.query);
  const response = await apiRequest({
    req,
    url: `${Endpoint.Announcement}?${params}`,
    method: "GET",
  });
