...
import { ApiResponseType, ICluster, IUnit,IUnitType } from '../../types';


export async function getCluster(req: NextApiRequest): Promise<[Response, ApiResponseType<ICluster>]> {
...
export async function createCluster(req: NextApiRequest) {
  const apiResponse = await apiRequest({ req, url: Endpoint.Klaster, method: 'POST', body: req.body });
...
export async function updateCluster(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.Klaster}/${id}`, method: 'PUT', body: req.body });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function deleteCluster(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.Klaster}/{property_cluter_id}?property_cluster_id=${id}`, method: 'DELETE' });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function updateUnit(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.Unit}/${id}`, method: 'PUT', body: req.body });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function getUnitType(req: NextApiRequest): Promise<[Response, ApiResponseType<IUnitType>]> {
...
export async function createUnitType(req: NextApiRequest) {
  const apiResponse = await apiRequest({ req, url: Endpoint.UnitType, method: 'POST', body: req.body });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function updateUnitType(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.UnitType}/${id}`, method: 'PUT', body: req.body });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function deleteUnitType(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.UnitType}/${id}`, method: 'DELETE' });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}