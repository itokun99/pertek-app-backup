...
export async function getProperty(req: NextApiRequest): Promise<[Response, ApiResponseType<IProperty>]> {
...
export async function createProperty(req: NextApiRequest) {
  const apiResponse = await apiRequest({ req, url: Endpoint.Property, method: 'POST', body: req.body });
...
export async function updateProperty(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.Property}/${id}`, method: 'PUT', body: req.body });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
...
export async function deleteProperty(req: NextApiRequest) {
  const { id } = req.query;
  const apiResponse = await apiRequest({ req, url: `${Endpoint.Property}/${id}`, method: 'DELETE' });
  const responseBody = await apiResponse.json();
  return [apiResponse, responseBody];
}
